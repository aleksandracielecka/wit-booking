...
public class ItemRepositoryImpl implements ItemRepository {
    ...
    public void addItem(ItemEntity item) {
        //otwarcie transakcji, dodanie i zamkniecie transakcji
    ...
    public Long findItemByName(String searchName) {
        TypedQuery<Long> items = entityManager
                .createQuery("select id from ItemEntity item where name=:nameParam"
                        , Long.class);
        items.setParameter("nameParam", searchName);

        try {

        return items.getSingleResult(); // tu beda dwa wyjatki, ktore trzeba obsluzyc
